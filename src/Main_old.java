...
public class Main_old {
    ...
    public static boolean knightTourSearchMethodOne(int boardSize) {
        ...
        nodes.add(new State(startingPosition[0], startingPosition[1], null, null));
        ...
        while (!queue.isEmpty()) {
            ...
            for (int i = 0; i < 8; i++) {
                ...
                if (isAvailable(newX, newY, boardSize)) {
                    State newState = new State(newX, newY, current, null);
    ...
    public static boolean preparationForDFS(int boardSize) {
        ...
        for (int startRow = 0; startRow < boardSize; startRow++) {
            for (int startCol = 0; startCol < boardSize; startCol++) {
                ...
                State startState = new State(startRow, startCol, null, null);
        ...
        if (method == 0) {
            for (int i = 0; i < MOVES.length; i++) {
                ...
                if (isSafe(nextRow, nextCol, chessBoard, N)) {
                    ...
                    State child = new State(nextRow, nextCol, current, null); // Create new state with parent
            ...
            while (!priorityQueue.isEmpty()) {
                ...
                State child = new State(nextBestRow, nextBestCol, current, null); // Create new state with parent
                State result = DFS(chessBoard, child, moveCount + 1, N); // Recursive call
                if (result != null) {
                    return result; // Solution found
                }
